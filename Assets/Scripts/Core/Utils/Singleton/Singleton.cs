...
public abstract class Singleton<T> : SingletonBase where T : Singleton<T>, new()
{
    ...
    public override IEnumerator InitCoroutine(UpdateProgressDelegate updateProgressHandler, int currentStep, int totalStep)
    {
        if (_instance == null)
            _instance = this as T;
        else
        ...
        yield return OnInitCoroutine(updateProgressHandler, currentStep, totalStep);
    }
...
public delegate IEnumerator UpdateProgressDelegate(string name, int step, int count, string subInfo);
...
public class SingletonBatch
{
    ...
    public IEnumerator InitCoroutine(UpdateProgressDelegate updateProgressHandler, int step, int count)
    {
        ...
        for (int i = 0, c = _singletons.Count; i < c; ++i)
        {
            ...
            yield return singleton.InitCoroutine(updateProgressHandler, step + i, count);

            yield return updateProgressHandler(singleton.ToString(), step + i, count, "finished");
        }
...
{
    SingletonBatch[] _batches = new SingletonBatch[5];
    int _count = 0;
