...
namespace Core.Utils.Log
{
    ...
    public class Log2Screen : MonoBehaviour
    {
        ...
        private Queue<LogMessage> _msgQueue = new Queue<LogMessage>(Log2ScreenBufSize);
        ...
        void OnGUI()
        {
            if (ShowSwitch)
            {
                if (GUI.Button(new Rect(0, 200, 100, 30), "显示/隐藏 日志"))
                {
                    IsVisible = !IsVisible;
                }
            }
        ...
        void HandleLog(string message, string stackTrace, LogType type)
        {
            ...
            while (_msgQueue.Count + 1 >= Log2ScreenBufSize)
                _msgQueue.Dequeue();
